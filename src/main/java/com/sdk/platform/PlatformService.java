...
public static class BillingService {
    ...
    public PlatformModels.CreateSubscriptionResponse createOneTimeSubscriptionCharge(String extensionId ,PlatformModels.CreateOneTimeSubscriptionCharge body) throws IOException {
        if (this.platformConfig.getPlatformOauthClient().isAccessTokenValid()) {
            Response<PlatformModels.CreateSubscriptionResponse> response = billingApiList.createOneTimeSubscriptionCharge(this.companyId , extensionId  , body).execute();
            if (!response.isSuccessful()) {
                    throw new IOException(response.errorBody() != null
                            ? response.errorBody().string() : Fields.UNKNOWN_ERROR);
                }
