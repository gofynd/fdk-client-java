...
public static class OrderInvoiceEngineService {
    ...
    public PlatformModels.StatusSuccessResponse getLabelStatus() throws IOException {
        if (this.platformConfig.getPlatformOauthClient().isAccessTokenValid()) {
            Response<PlatformModels.StatusSuccessResponse> response = orderinvoiceengineApiList.getLabelStatus(this.companyId ).execute();
            if (!response.isSuccessful()) {
                    throw new IOException(response.errorBody() != null
                            ? response.errorBody().string() : Fields.UNKNOWN_ERROR);
                }
    ...
    public PlatformModels.SignedSuccessResponse getLabelPresignedURL() throws IOException {
        if (this.platformConfig.getPlatformOauthClient().isAccessTokenValid()) {
            Response<PlatformModels.SignedSuccessResponse> response = orderinvoiceengineApiList.getLabelPresignedURL(this.companyId ).execute();
            if (!response.isSuccessful()) {
                    throw new IOException(response.errorBody() != null
                            ? response.errorBody().string() : Fields.UNKNOWN_ERROR);
                }
