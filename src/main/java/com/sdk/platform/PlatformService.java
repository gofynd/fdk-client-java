...
public class ApplicationClient {
    ...
    public PlatformModels.UserGroupListResponseSchema getUserGroups(String pageNo , String pageSize , String name , String status , Integer groupUid ) throws IOException {
        if (this.platformConfig.getPlatformOauthClient().isAccessTokenValid()) {
            Response<PlatformModels.UserGroupListResponseSchema> response = userApiList.getUserGroups(this.companyId , this.applicationId ,pageNo , pageSize , name , status , groupUid ).execute();
            if (!response.isSuccessful()) {
                    throw new IOException(response.errorBody() != null
                            ? response.errorBody().string() : Fields.UNKNOWN_ERROR);
                }
