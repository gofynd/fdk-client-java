...
public class ApplicationClient {
    ...
    public PlatformModels.GetUserCODLimitResponse getUserCODlimitRoutes() throws IOException {
        if (this.platformConfig.getPlatformOauthClient().isAccessTokenValid()) {
            Response<PlatformModels.GetUserCODLimitResponse> response = paymentApiList.getUserCODlimitRoutes(this.companyId , this.applicationId ).execute();
            if (!response.isSuccessful()) {
                    throw new IOException(response.errorBody() != null
                            ? response.errorBody().string() : Fields.UNKNOWN_ERROR);
                }
