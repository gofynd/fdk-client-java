...
public static class OrdersService {
    ...
    public PlatformModels.MetricCountResponse getMetricCount() throws IOException {
        if (this.platformConfig.getPlatformOauthClient().isAccessTokenValid()) {
            Response<PlatformModels.MetricCountResponse> response = ordersApiList.getMetricCount(this.companyId ).execute();
            if (!response.isSuccessful()) {
                    throw new IOException(response.errorBody() != null
                            ? response.errorBody().string() : Fields.UNKNOWN_ERROR);
                }
