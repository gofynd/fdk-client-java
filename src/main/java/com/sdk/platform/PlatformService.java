...
public class ApplicationClient {
    ...
    **/
    ...
    public PlatformModels.OverrideCheckoutResponse overrideCart(PlatformModels.OverrideCheckoutReq body) throws IOException {
        if (this.platformConfig.getPlatformOauthClient().isAccessTokenValid()) {
            Response<PlatformModels.OverrideCheckoutResponse> response = cartApiList.overrideCart(this.companyId , this.applicationId , body).execute();
            if (!response.isSuccessful()) {
                    throw new IOException(response.errorBody() != null
                            ? response.errorBody().string() : Fields.UNKNOWN_ERROR);
                }
