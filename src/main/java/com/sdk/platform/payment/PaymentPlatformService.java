...
public class PaymentPlatformService {
    ...
    public PaymentPlatformModels.PayoutPennyDropAndChequePayload getPayoutPennyDropAndChequeConfig() throws FDKServerResponseError, FDKException {
        if (this.platformConfig.getPlatformOauthClient().isAccessTokenValid()) {
            Response<PaymentPlatformModels.PayoutPennyDropAndChequePayload> response = null;
            try {
                response = paymentPlatformApiList.getPayoutPennyDropAndChequeConfig(this.companyId ).execute();
                if (!response.isSuccessful()) {
                    throw new FDKServerResponseError(response.code(),
                                            response.errorBody() != null ? response.errorBody().string() : Fields.UNKNOWN_ERROR,
                                            response.headers() != null ? response.headers().toString() : Fields.UNKNOWN_ERROR,
