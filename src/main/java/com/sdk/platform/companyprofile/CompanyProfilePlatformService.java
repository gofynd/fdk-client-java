...
public class CompanyProfilePlatformService {
    ...
    **/
    ...
    public CompanyProfilePlatformModels.AssignStoreResponseSerializer getOptimalLocations(CompanyProfilePlatformModels.AssignStoreRequestValidator body) throws FDKServerResponseError, FDKException {
        if (this.platformConfig.getPlatformOauthClient().isAccessTokenValid()) {
            Response<CompanyProfilePlatformModels.AssignStoreResponseSerializer> response = null;
            try {
                response = companyprofilePlatformApiList.getOptimalLocations(this.companyId , body).execute();
                if (!response.isSuccessful()) {
                    throw new FDKServerResponseError(response.code(),
                                            response.errorBody() != null ? response.errorBody().string() : Fields.UNKNOWN_ERROR,
                                            response.headers() != null ? response.headers().toString() : Fields.UNKNOWN_ERROR,
