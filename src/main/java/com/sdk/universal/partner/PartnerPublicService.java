...
public class PartnerPublicService {
    ...
    public PartnerPublicModels.OrganizationList getOrganizationList() throws IOException {
    
        Response<PartnerPublicModels.OrganizationList> response = partnerPublicApiList.getOrganizationList().execute();
        if(!response.isSuccessful()) {
            throw new IOException(response.errorBody() != null
                    ? response.errorBody().string() : Fields.UNKNOWN_ERROR);
        }
