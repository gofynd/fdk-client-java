...
interface LeadPartnerApiList {
    ...
    Call<LeadPartnerModels.TicketList> getTickets(@Path("organization_id") String organizationId, @Query("items") Boolean items, @Query("filters") Boolean filters, @Query("q") String q, @Query("status") String status, @Query("priority") LeadPartnerModels.PriorityEnum priority, @Query("category") String category, @Query("page_no") Integer pageNo, @Query("page_size") Integer pageSize, @HeaderMap Map<String, String> requestHeaders);
