...
@Getter
 public class OrderService { 
    ...
    public ApplicationModels.ResponseGetInvoiceShipment1 getInvoiceByShipmentId(String shipmentId , ApplicationModels.invoiceParameter parameters ) throws IOException {
      ...
      String fullUrl = relativeUrls.get("getInvoiceByShipmentId");
        ...
        Response<ApplicationModels.ResponseGetInvoiceShipment1> response = orderApiList.getInvoiceByShipmentId(fullUrl  ,parameters).execute();
        if(!response.isSuccessful()) {
            throw new IOException(response.errorBody() != null
                    ? response.errorBody().string() : Fields.UNKNOWN_ERROR);
        }
