...
@Getter
 public class ThemeApplicationService { 
    ...
    public ThemeApplicationModels.AvailablePageSchema getPage(String themeId, String pageValue, String filters, String sectionPreviewHash, Integer company, String urlParams) throws IOException {
        return this.getPage(themeId, pageValue, filters, sectionPreviewHash, company, urlParams, new HashMap<>());
    }

    public ThemeApplicationModels.AvailablePageSchema getPage(String themeId, String pageValue, String filters, String sectionPreviewHash, Integer company, String urlParams, Map<String, String> requestHeaders) throws IOException {
     
        String fullUrl = relativeUrls.get("getPage");
        fullUrl = fullUrl.replace("{" + "theme_id" + "}",themeId.toString());
        fullUrl = fullUrl.replace("{" + "page_value" + "}",pageValue.toString());

        Response<ThemeApplicationModels.AvailablePageSchema> response = themeApplicationApiList.getPage(fullUrl, filters, sectionPreviewHash, company, urlParams, requestHeaders).execute();
        if(!response.isSuccessful()) {
            throw new IOException(response.errorBody() != null
                    ? response.errorBody().string() : Fields.UNKNOWN_ERROR);
        }
