...
@Getter
 public class CartApplicationService { 
    ...
    public CartApplicationModels.AddCartDetailResponse addItems(Boolean i , Boolean b , String areaCode , String id , Boolean buyNow ,CartApplicationModels.AddCartRequest body) throws IOException {
     
      String fullUrl = relativeUrls.get("addItems");
        

        Response<CartApplicationModels.AddCartDetailResponse> response = cartApplicationApiList.addItems(fullUrl  ,i, b, areaCode, id, buyNow, body).execute();
        if(!response.isSuccessful()) {
            throw new IOException(response.errorBody() != null
                    ? response.errorBody().string() : Fields.UNKNOWN_ERROR);
        }
