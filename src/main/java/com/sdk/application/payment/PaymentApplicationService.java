...
@Getter
 public class PaymentApplicationService { 
    ...
    public PaymentApplicationService(ApplicationConfig applicationConfig) {
        ...
        relativeUrls.put("getOutagesData","/service/application/payment/v1.0/payment/outages".substring(1));
    ...
    public PaymentApplicationModels.PaymentOutageResponseData getOutagesData(String aggregator, Boolean refresh, List<String> mopfilter) throws IOException {
        return this.getOutagesData(aggregator, refresh, mopfilter, new HashMap<>());
    }

    public PaymentApplicationModels.PaymentOutageResponseData getOutagesData(String aggregator, Boolean refresh, List<String> mopfilter, Map<String, String> requestHeaders) throws IOException {
     
        String fullUrl = relativeUrls.get("getOutagesData");

        Response<PaymentApplicationModels.PaymentOutageResponseData> response = paymentApplicationApiList.getOutagesData(fullUrl, aggregator, refresh, mopfilter, requestHeaders).execute();
        if(!response.isSuccessful()) {
            throw new IOException(response.errorBody() != null
                    ? response.errorBody().string() : Fields.UNKNOWN_ERROR);
        }
    ...
    public PaymentApplicationModels.CheckCreditDetails checkCredit(String aggregator, String wallet) throws IOException {
        return this.checkCredit(aggregator, wallet, new HashMap<>());
    }

    public PaymentApplicationModels.CheckCreditDetails checkCredit(String aggregator, String wallet, Map<String, String> requestHeaders) throws IOException {
     
        String fullUrl = relativeUrls.get("checkCredit");

        Response<PaymentApplicationModels.CheckCreditDetails> response = paymentApplicationApiList.checkCredit(fullUrl, aggregator, wallet, requestHeaders).execute();
        if(!response.isSuccessful()) {
            throw new IOException(response.errorBody() != null
                    ? response.errorBody().string() : Fields.UNKNOWN_ERROR);
        }
