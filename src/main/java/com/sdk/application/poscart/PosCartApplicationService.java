...
@Getter
 public class PosCartApplicationService { 
    ...
    public PosCartApplicationModels.AddCartDetailResponse addItems(Boolean i , Boolean b , String areaCode ,PosCartApplicationModels.AddCartRequest body) throws IOException {
     
      String fullUrl = relativeUrls.get("addItems");
        

        Response<PosCartApplicationModels.AddCartDetailResponse> response = poscartApplicationApiList.addItems(fullUrl  ,i, b, areaCode, body).execute();
        if(!response.isSuccessful()) {
            throw new IOException(response.errorBody() != null
                    ? response.errorBody().string() : Fields.UNKNOWN_ERROR);
        }
