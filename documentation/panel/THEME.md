...
```java
panelClient.theme.getMarketplaceThemes( pageSize,  pageNo) {
  //use response
}
```
...
| --------- | -----  | -------- | ----------- | 
| pageSize | Integer? | no | Number of themes per page |   
| pageNo | Integer? | no | Page number to retrieve |  
...
```java
panelClient.theme.getMarketplaceThemeBySlug( slugName) {
  //use response
}
```
...
```java
panelClient.theme.getMarketplaceThemeVersions( slugName) {
  //use response
}
```
