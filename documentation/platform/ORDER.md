...
```java
platformClient.order.postRefundStateConfiguration( appId, body body) {
  //use response
}
```
...
```java
platformClient.order.getRefundStateConfiguration( appId) {
  //use response
}
```
...
```java
platformClient.order.getRefundEnableStateList() {
  //use response
}
```
...
```java
platformClient.order.postRefundConfiguration( appId, body body) {
  //use response
}
```
...
```java
platformClient.order.getRefundConfiguration( appId) {
  //use response
}
```
...
```java
platformClient.application("<APPLICATION_ID>").order.getQuestions( pageNo,  pageSize,  q,  isActive) {
  //use response
}
```
...
```java
platformClient.application("<APPLICATION_ID>").order.getRuleLaneConfig( searchValue) {
  //use response
}
```
...
```java
platformClient.application("<APPLICATION_ID>").order.createRule(body body) {
  //use response
}
```
...
```java
platformClient.application("<APPLICATION_ID>").order.getRuleById( ruleId) {
  //use response
}
```
...
```java
platformClient.application("<APPLICATION_ID>").order.updateRule( ruleId, body body) {
  //use response
}
```
...
```java
platformClient.application("<APPLICATION_ID>").order.deleteRule( ruleId) {
  //use response
}
```
...
```java
platformClient.application("<APPLICATION_ID>").order.updateRulePosition(body body) {
  //use response
}
```
...
```java
platformClient.application("<APPLICATION_ID>").order.getRuleParameters() {
  //use response
}
```
...
```java
platformClient.application("<APPLICATION_ID>").order.getRuleList(body body) {
  //use response
}
```
...
```java
platformClient.order.getOrders( lane,  searchType,  bagStatus,  timeToDispatch,  paymentMethods,  tags,  searchValue,  fromDate,  toDate,  startDate,  endDate,  dpIds,  stores,  salesChannels,  pageNo,  pageSize,  isPrioritySort,  customMeta,  myOrders,  showCrossCompanyData,  customerId,  orderType) {
  //use response
}
```
...
| timeToDispatch | Integer? | no | time_to_dispatch refers to estimated SLA time. |   
| paymentMethods | String? | no |  |   
...
```java
platformClient.order.getuserviews() {
  //use response
}
```
...
```java
platformClient.order.postuserviews(body body) {
  //use response
}
```
...
```java
platformClient.order.updateuserviews(body body) {
  //use response
}
```
...
```java
platformClient.order.deleteuserviews( id) {
  //use response
}
```
...
```java
platformClient.order.globalfilters( showIn,  requestSource) {
  //use response
}
```
...
```java
platformClient.order.getBulkShipmentExcelFile( salesChannels,  dpIds,  startDate,  endDate,  stores,  tags,  bagStatus,  paymentMethods,  fileType,  timeToDispatch,  pageNo,  pageSize) {
  //use response
}
```
...
```java
platformClient.order.generatePOSReceiptByOrderId( orderId,  shipmentId,  documentType) {
  //use response
}
```
...
```java
platformClient.order.getAllowedTemplatesForBulk() {
  //use response
}
```
...
```java
platformClient.order.getTemplate( templateName) {
  //use response
}
```
...
---
 ...
 | description | String? |  yes  |  |
 | type | String? |  yes  |  |
 | options | HashMap<String,Object>? |  yes  |  |
...
---
 ...
 | kwargs | HashMap<String,Object>? |  yes  |  |
...
---
 ...
 | Properties | Type | Nullable | Description |
 | ---------- | ---- | -------- | ----------- |
 | id | Integer? |  yes  | Unique identifier for the state |
...
---
 ...
 | ---------- | ---- | -------- | ----------- |
 | base | String? |  yes  |  |
 | rates | HashMap<String,Object>? |  yes  |  |
...
---
 ...
 | invoiceId | String? |  yes  |  |
 | paymentMethods | HashMap<String,Object>? |  yes  |  |
 | paymentInfo | ArrayList<HashMap<String,Object>>? |  yes  |  |
 | statusCreatedAt | String? |  yes  |  |
 | statusCreatedTs | String? |  yes  |  |
 | displayName | String? |  yes  |  |
...
---
 ...
 | displayName | String? |  yes  |  |
 | dpDetails | HashMap<String,Object>? |  yes  |  |
 | einvoiceInfo | HashMap<String,Object>? |  yes  |  |
 | entityType | String? |  yes  |  |
 | fallbackUser | HashMap<String,Object>? |  yes  |  |
