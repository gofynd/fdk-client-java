...
```java
client.order.getOrders( lane,  searchType,  searchValue,  fromDate,  toDate,  dpIds,  stores,  salesChannels,  pageNo,  pageSize,  isPrioritySort,  customMeta) {
  //use response
}
```
...
```java
client.order.generatePOSReceiptByOrderId( orderId) {
  //use response
}
```
