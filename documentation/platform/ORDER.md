...
```java
client.order.getAssetByShipmentIds( shipmentIds,  invoice) {
  //use response
}
```
...
```java
client.order.getOrders( lane,  searchType,  bagStatus,  timeToDispatch,  paymentMethods,  tags,  searchValue,  fromDate,  toDate,  dpIds,  stores,  salesChannels,  orderingStore,  pageNo,  pageSize,  isPrioritySort,  customMeta) {
  //use response
}
```
...
```java
client.order.generatePOSReceiptByOrderId( orderId,  documentType) {
  //use response
}
```
