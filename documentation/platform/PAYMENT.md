...
```java
client.application("<APPLICATION_ID>").payment.getPosPaymentModeRoutes() {
  //use response
}
```
...
```java
client.application("<APPLICATION_ID>").payment.getPosPaymentModeRoutes(body body) {
  //use response
}
```
...
---
 ...
 | remaining | Integer |  no  | Remaining Limit for COD of User |
