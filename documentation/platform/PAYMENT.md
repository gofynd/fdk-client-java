...
```java
client.application("<APPLICATION_ID>").payment.oauthGetUrl( aggregator,  successRedirectUrl,  failureRedirectUrl) {
  //use response
}
```
...
```java
client.application("<APPLICATION_ID>").payment.revokeOauthToken( aggregator) {
  //use response
}
```
