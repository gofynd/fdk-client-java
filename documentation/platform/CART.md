...
```java
platformClient.application("<APPLICATION_ID>").cart.updateCartDynamicInjection( extensionId, body body) {
  //use response
}
```
...
```java
platformClient.application("<APPLICATION_ID>").cart.removeCartMetaConfig( extensionId) {
  //use response
}
```
