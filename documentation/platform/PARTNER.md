...
```java
platformClient.partner.subscribeExtension( entity,  extensionId,  entityId, body body) {
  //use response
}
```
...
```java
platformClient.partner.getPublicExtension( extensionId) {
  //use response
}
```
...
```java
platformClient.partner.getExtensionById( extensionId) {
  //use response
}
```
...
```java
platformClient.partner.deleteExtensionById( extensionId,  message,  uninstallReasonType) {
  //use response
}
```
...
```java
platformClient.partner.getExtensionsSuggestions( pageSize) {
  //use response
}
```
...
```java
platformClient.partner.getPartnerInvites( requestStatus,  pageSize,  pageNo) {
  //use response
}
```
...
```java
platformClient.partner.getPartnerRequestDetails( inviteId) {
  //use response
}
```
...
```java
platformClient.partner.modifyPartnerRequest( inviteId, body body) {
  //use response
}
```
...
```java
platformClient.partner.setupProducts( requestId) {
  //use response
}
```
...
```java
platformClient.application("<APPLICATION_ID>").partner.getProxyPath( extensionId) {
  //use response
}
```
...
```java
platformClient.application("<APPLICATION_ID>").partner.getProxyPathAttachedPath( extensionId,  attachedPath) {
  //use response
}
```
