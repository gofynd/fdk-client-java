...
```java
client.catalog.getSizeGuides( active,  q,  tag,  pageNo,  pageSize,  brandId) {
  //use response
}
```
...
| brandId | Integer? | no | to filter size guide on basis of brand_id. |  
...
```java
client.catalog.listDepartmentsData( pageNo,  pageSize,  name,  search,  isActive,  itemType) {
  //use response
}
```
...
```java
client.catalog.listProductTemplateExportDetails( status,  fromDate,  toDate,  q) {
  //use response
}
```
...
```java
client.catalog.patchProduct( itemId, body body) {
  //use response
}
```
...
| --------- | -----  | -------- | ----------- | 
| companyId | String | yes | Id of the company associated to product that is to be viewed. |   
| itemId | Integer | yes | Id of the product to be patch. |  
...
```java
client.catalog.getInventoryExport( status,  fromDate,  toDate,  q) {
  //use response
}
```
...
---
 ...
 | attributesSchema | ArrayList<HashMap<String,Object>>? |  yes  |  |
