...
```java
platformClient.application("<APPLICATION_ID>").catalog.getProductSizeSellers( itemId,  size,  pincode,  strategy,  pageNo,  pageSize) {
  //use response
}
```
...
| companyId | Integer | yes | Company id of the product size . |   
| applicationId | String | yes | Application id of company for which product size to be fetched. |   
...
```java
platformClient.catalog.deleteRealtimeInventory( itemId,  sellerIdentifier) {
  //use response
}
```
