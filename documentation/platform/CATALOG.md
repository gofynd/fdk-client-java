...
```java
client.application("<APPLICATION_ID>").catalog.getAllCollections( q,  type,  tags,  isActive,  pageNo,  pageSize) {
  //use response
}
```
...
```java
client.catalog.getProductAttributes( q,  department,  category,  filtered,  pageNo,  pageSize,  sort) {
  //use response
}
```
...
| pageNo | Integer? | no | The number of page |   
| pageSize | Integer? | no | The size of each page. |   
| sort | Integer? | no | The type of sort to sort the list of attributes. |  
