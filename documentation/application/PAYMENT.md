...
```java
applicationClient.payment.getPaymentModeRoutes( amount,  cartId,  checkoutMode,  refresh,  orderId,  cardReference,  userDetails,  displaySplit,  advancePayment,  shipmentId) {
  //use response
}
```
...
```java
applicationClient.payment.walletLinkInitate(body body) {
  //use response
}
```
...
```java
applicationClient.payment.linkWallet(body body) {
  //use response
}
```
...
```java
applicationClient.payment.delinkWallet(body body) {
  //use response
}
```
...
```java
applicationClient.payment.cardDetails( cardInfo,  aggregator) {
  //use response
}
```
...
```java
applicationClient.payment.outstandingOrderDetails( aggregator) {
  //use response
}
```
...
```java
applicationClient.payment.paidOrderDetails( aggregator) {
  //use response
}
```
...
```java
applicationClient.payment.createPaymentOrder(body body) {
  //use response
}
```
...
```java
applicationClient.payment.deleteBeneficiaryDetails( beneficiaryId) {
  //use response
}
```
...
```java
applicationClient.payment.getRefundOptions( configuration,  productType,  amount) {
  //use response
}
```
...
```java
applicationClient.payment.setRefundOptionforShipment(body body) {
  //use response
}
```
...
```java
applicationClient.payment.getSelectedRefundOption( shipmentId,  orderId) {
  //use response
}
```
...
```java
applicationClient.payment.getUserBeneficiariesDetailV2( orderId,  shipmentId,  mop) {
  //use response
}
```
...
```java
applicationClient.payment.validateBeneficiaryAddress(body body) {
  //use response
}
```
...
---
 ...
 | meta | HashMap<String,Object>? |  yes  | Payment methods meta |
...
---
 ...
 | otp | Integer |  no  | OTP received for wallet linking |
...
---
 ...
 | Properties | Type | Nullable | Description |
 | ---------- | ---- | -------- | ----------- |
 | data | HashMap<String,Object> |  no  | Response received from aggregator |
...
---
 ...
 | shopAndEstablishment | HashMap<String,Object>? |  yes  | Shop Establishment |
...
---
 ...
 | session | HashMap<String,Object>? |  yes  | User Session |
...
---
 ...
 | ---------- | ---- | -------- | ----------- |
 | statusCode | Integer |  no  | HTTP Status code |
 | data | ArrayList<HashMap<String,Object>>? |  yes  | Dict containing the outstanding order details |
 | success | Boolean |  no  | Response is successful or not. |
 | message | String? |  yes  | Message |
...
---
 ...
 | ---------- | ---- | -------- | ----------- |
 | statusCode | Integer |  no  | HTTP Status code |
 | data | ArrayList<HashMap<String,Object>>? |  yes  | Dict containing the paid order details |
 | success | Boolean |  no  | Response is successful or not. |
 | message | String? |  yes  | Message |
