...
```java
catalog.getCollections( pageNo,  pageSize,  tag,  q) {
  //use response
}
```
...
```java
catalog.getCollectionItemsBySlug( slug,  f,  q,  filters,  sortOn,  pageId,  pageSize,  pageNo,  pageType) {
  //use response
}
```
...
```java
catalog.getProductPriceBySlug( slug,  size,  storeId,  pincode,  moq) {
  //use response
}
```
