...
```java
order.getOrders( status,  pageNo,  pageSize,  fromDate,  toDate) {
  //use response
}
```
...
```java
order.getInvoiceByShipmentId( shipmentId,  parameters) {
  //use response
}
```
...
```java
order.getCreditNoteByShipmentId( shipmentId,  parameters) {
  //use response
}
```
