...
```java
order.getOrders( status,  pageNo,  pageSize,  fromDate,  toDate,  customMeta) {
  //use response
}
```
...
```java
order.updateShipmentExternal( shipmentId, body body) {
  //use response
}
```
...
```java
order.getInvoiceByShipmentId1( shipmentId,  parameters) {
  //use response
}
```
...
---
 ...
 | statuses | ArrayList<HashMap<String,Object>> |  no  |  |
