...
### getTickets
Gets the list of partner level tickets and/or ticket filters depending on query params




```java
partnerClient.lead.getTickets( organizationId,  items,  filters,  q,  status,  priority,  category,  pageNo,  pageSize) {
  //use response
}
```
...
| --------- | -----  | -------- | ----------- | 
| organizationId | String | yes | Partner ID for which the data will be returned |   
...
```java
partnerClient.lead.createTicket( organizationId, body body) {
  //use response
}
```
...
### getTicket
Retreives ticket details of a partner level ticket with ticket ID




```java
partnerClient.lead.getTicket( organizationId,  id) {
  //use response
}
```
...
### editTicket
Edits ticket details of a partner level ticket




```java
partnerClient.lead.editTicket( organizationId,  id, body body) {
  //use response
}
```
...
| Argument  |  Type  | Required | Description |
| --------- | -----  | -------- | ----------- | 
| organizationId | String | yes | Partner ID for ticket |   
...
### createHistory
Create history for specific partner level ticket




```java
partnerClient.lead.createHistory( organizationId,  id, body body) {
  //use response
}
```
...
### getTicketHistory
Gets history list for specific partner level ticket




```java
partnerClient.lead.getTicketHistory( organizationId,  id) {
  //use response
}
```
    ...
    "docs": [
      {
        ...
        "created_by": {
          ...
          "accountType": "user",
          "active": true,
          "profilePicUrl": "https://cdn.fynd.com/v2/falling-surf-7c8bb8/fyndnp/wrkr/addsale/partners/pictures/free/original/--unnamed--/banner.jpeg",
          "hasOldPasswordHash": false,
...
```java
partnerClient.lead.getTokenForVideoRoom( organizationId,  uniqueName) {
  //use response
}
```
...
| Argument  |  Type  | Required | Description |
| --------- | -----  | -------- | ----------- | 
| organizationId | String | yes | Partner Id for video room |   
...
```java
partnerClient.lead.getVideoParticipants( organizationId,  uniqueName) {
  //use response
}
```
...
```java
partnerClient.lead.getGeneralConfig( organizationId) {
  //use response
}
```
...
---
 ...
 | source | String? |  yes  | Denotes if the ticket was created at partner or application level |
...
---
 ...
 | Properties | Type | Nullable | Description |
 | ---------- | ---- | -------- | ----------- |
 | supportCommunication | ArrayList<[SupportCommunicationSchema](#SupportCommunicationSchema)>? |  yes  |  |
 | type | String? |  yes  |  |
 | integration | [GeneralConfigIntegrationSchema](#GeneralConfigIntegrationSchema)? |  yes  |  |
 | availableIntegration | ArrayList<String>? |  yes  |  |
...
---
 ...
 | partnerId | String |  no  | Partner ID related to the ticket |
...
---
 ...
 | partnerId | String |  no  | Partner id for which ticket was raised |
...
---
 ...
 | createdOn | [CreatedOn](#CreatedOn)? |  yes  |  |
 | responseId | String? |  yes  | Details of partner and application realated to the ticket |
 | content | [TicketContent](#TicketContent)? |  yes  |  |
...
---
 ...
 | Name | Value | Description |
 | ---- | ----- | ----------- |
 | platformPanel | platform_panel | This means it is partner level ticket |
